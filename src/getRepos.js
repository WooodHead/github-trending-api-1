    ...
    fetch(FETCH_URL)
      ...
      .then(body => {
        ...
        let result = box.get().map(el => {
          ...
          const [author, reponame] = $(el).find('.h3>a').text().split('/').map(v => v.trim());
          ...
          const langcolor = color ? color.replace('background-color: ', '') : '';

          let builtby = $(el).find('span:contains("Built by")>a').map((i, user) => {
